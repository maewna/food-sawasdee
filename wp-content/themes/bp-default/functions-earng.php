...
function setmenu_taxonomy() {  
    register_taxonomy(  
        'setmenu_categories',  //The name of the taxonomy. Name should be in slug form (must not contain capital letters or spaces). 
        'setmenu',        //post type name
        array(  
            'hierarchical' => true,  
            'label' => 'Setmenu Categories',  //Display name
            'query_var' => true,
            'rewrite' => array(
                'slug' => 'setmenu', // This controls the base slug that will display before each term
                'with_front' => false // Don't display the category base before 
            )
        )  
    );  
}  
add_action( 'init', 'setmenu_taxonomy');



function filter_post_type_link($link, $post)
{
    if ($post->post_type != 'setmenu')
        return $link;

    if ($cats = get_the_terms($post->ID, 'setmenu_categories'))
        $link = str_replace('%setmenu_categories%', array_pop($cats)->slug, $link);
...
function default_taxonomy_term( $post_id, $post ) {
    if ( 'publish' === $post->post_status ) {
        $defaults = array(
            'setmenu_categories' => array( 'other'),   // set default category
            );
...
function create_post_type() {
	...
	register_post_type( 'setmenu',
        array(
			...
			'taxonomies' => array( 'post_tag','setmenu_categories'),
			'rewrite' => array('slug' => 'setmenu/%setmenu_categories%','with_front' => FALSE),
			'has_archive' => 'setmenu'
...
function update_checkbox_meal( $post_id ) {   
	if ( isset( $_POST['checkbox_meal1'] ) ) {
		...
		$_POST['checkbox_meal1'] = array('breakfast','lunch','dinner','supper');
		update_post_meta( $post_id, 'check_meal', $_POST['checkbox_meal1'] );
	}
}
...
function render_checkbox_type ( $form_id, $post_id, $form_settings ) {
	$value = array();
	$types = array('alacarte','maincourse','cookedtoorder','snacks','fusion','vegetarian');
	$typesTH = array('อาหารจานเดียว','อาหารจานหลัก','อาหารตามสั่ง','อาหารว่าง','อาหารฟิวชั่น','อาหารมังสวิรัติ / เจ');
		if ( $post_id ) { $value = get_post_meta($post_id, 'recipe_type', true); }
    ?>
	<div class="wpuf-label"> <label> ประเภทของอาหาร </label> </div>
	<div class="wpuf-fields">
		<div class="mealRating">
			<?php
			$count = 0;
			 foreach ($types as $type) {
				   ...
				   echo '<input type="checkbox" name="checkbox_type1[]" id="checkbox-'.$count.'"  value="'.$type.'"'  . $checked . '/>
						<label for="checkbox-'.$count.'"" >'.$typesTH[$count].'</label>';
					$count++;
			   }
			?>
...
function render_recipeNational( $form_id, $post_id, $form_settings ) { 
    if ( $post_id ) {
		...
		$chkreg_thfood = get_post_meta($post_id, 'regis_thaifood', true);
    }
    ?>
<script src="//code.jquery.com/jquery-1.10.2.js"></script>
    ...
    <div class="wpuf-fields">
		<div class="recipeNational">
					<input type="radio" name="radio_national" value="inter" <?php checked( $value, 'inter' ); ?>>
					<label for="national1"><span>นานาชาติ</span></label>
					<input type="radio" name="radio_national" id="thai-rdo" value="thai" <?php checked( $value, 'thai' ); ?>>
					<label for="national2"><span>ไทย</span></label>
					<input type="radio" name="radio_national" value="china" <?php checked( $value, 'china' ); ?>>
					<label for="national3"><span>จีน</span></label>
					<input type="radio" name="radio_national" value="japan" <?php checked( $value, 'japan' ); ?>>
					<label for="national4"><span>ญี่ปุ่น</span></label>
					<input type="radio" name="radio_national" value="korea" <?php checked( $value, 'korea' ); ?>>
					<label for="national5"><span>เกาหลี</span></label>
					<input type="radio" name="radio_national" value="vietnam" <?php checked( $value, 'vietnam' ); ?>>
					<label for="national6"><span>เวียดนาม</span></label>
					<input type="radio" name="radio_national" value="italy" <?php checked( $value, 'italy' ); ?>>
					<label for="national7"><span>อิตาเลียน</span></label>
					<input type="radio" name="radio_national" value="france" <?php checked( $value, 'france' ); ?>>
					<label for="national8"><span>ฝรั่งเศส</span></label>
					<input type="radio" name="radio_national" value="america" <?php checked( $value, 'america' ); ?>>
					<label for="national9"><span>อเมริกัน</span></label>
					<input type="radio" name="radio_national" value="india" <?php checked( $value, 'india' ); ?>>
					<label for="national10"><span>อินเดีย</span></label>
		</div>
    </div>
	...
	<div class="wpuf-fields">
		<div class="recipeThai">
					<input type="radio" name="radio_thaifood" value="central" <?php checked( $value1, 'central' ); ?>>
					<label for="thaifood1"><span>ภาคกลาง</span></label>
					<input type="radio" name="radio_thaifood" value="south" <?php checked( $value1, 'south' ); ?>>
					<label for="thaifood2"><span>ภาคใต้</span></label>
					<input type="radio" name="radio_thaifood" value="north" <?php checked( $value1, 'north' ); ?>>
					<label for="thaifood3"><span>ภาคเหนือ</span></label>
					<input type="radio" name="radio_thaifood" value="east" <?php checked( $value1, 'east' ); ?>>
					<label for="thaifood4"><span>ภาคอีสาน</span></label>
		</div>
	</div>
	<div class="wpuf-fields">
		<input type="checkbox" name="checkbox_thaifood" id="chkreg-thaifood" value="yes" <?php checked( $chkreg_thfood, 'yes' ); ?>/>
		<label for="chkreg-thaifood" >ท่านต้องการสมัครเข้าร่วมเมนูอาหารรักประเทศไทย<br/>* ต้องผ่านการตรวจสอบจาก Admin ของ Foodsawasdee<br/>* หากมีการแก้ไขบทความ ต้องผ่านการตรวจสอบจาก Admin ของ Foodsawasdee ใหม่อีกครั้ง</label>
	</div>
	</div>
    <?php	
}
...
function update_regisThaiFood( $post_id ) {
    if ( isset( $_POST['checkbox_thaifood'] ) ) {
        update_post_meta( $post_id, 'regis_thaifood', $_POST['checkbox_thaifood'] );
    }else{
		update_post_meta( $post_id, 'regis_thaifood', '' );
	}
}
...
add_action( 'wpuf_add_post_after_insert', 'update_regisThaiFood' );
add_action( 'wpuf_edit_post_after_update', 'update_regisThaiFood' );
...
function manage_recipes_columns( $columns ) {
		...
		if($post_type == 'recipes'){
			...
			$columns["regis_thaifood"] = "สมัครเมนูรักประเทศไทย";
...
function my_recipes_column( $colname, $cptid ) {
	switch ($colname) {
		...
		case 'region_thaifood': ?>  
			 ...
			 <?php break;
		case 'regis_thaifood': ?>  
			 <?php echo get_post_meta( $cptid, 'regis_thaifood', true ); ?>   
			 <?php break;
		case 'admin_approve': ?>  
...
function lovethailand_restrict_posts_by_metavalue() {
	...
	$selected = $_GET['status_thaifood'];
	
    if ($typenow == 'recipes') {    
        $output = '<select style="width:150px" name="status_thaifood" class="postform">';
        $output .= '<option '.selected($selected,0,false).' value="">'.__('Show All','').'</option>';
        $output .= '<option '.selected($selected,"request",false).' value="request">เมนูที่ขอเข้าร่วมรักประเทศไทย</option>';    
		$output .= '<option '.selected($selected,"approved",false).' value="approved">เมนูที่ได้รับการอนุมัติแล้ว</option>';  
		$output .= '<option '.selected($selected,"pending",false).' value="pending">เมนูที่รอการอนุมัติ</option>';  
        $output .= '</select>';       
    echo $output;
    }
}
...
function lovethailand_pre_get_posts( $query ) {
    ...
    $lovethailand_str = $query->get('status_thaifood');
    if( !empty($lovethailand_str) ){
		if( $lovethailand_str == 'request' ){
			$meta_query[] = array(
					'key' => 'regis_thaifood',
					'value' => 'yes',
					'compare' => 'LIKE',
			);
		}else if( $lovethailand_str == 'approved' ){
			$meta_query = array(
				'relation' => 'AND',
				array(
					'key' => 'admin_approve',
		...
		}else if( $lovethailand_str == 'pending' ){
			$meta_query = array(
				'relation' => 'AND',
				array(
					'key' => 'admin_approve',
...
function bp_activity_at_mentionn_add_notification($post_id) {
    if ( bp_is_active( 'notifications' ) ) {
		...
		if (  $followers = bp_follow_get_followers( array( 'user_id' => $post_author_id  ) ) ) {
				foreach ($followers as $follower) {
					bp_notifications_add_notification( array(
						'user_id'           => $follower,//คนที่ได้ noti
						'item_id'           => $post_author_id,//คนเขียน
						'secondary_item_id' => $post_id,
						'component_name'    => 'activity',
						'component_action'  => 'new_at_post',
