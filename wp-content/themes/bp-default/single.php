<?php get_header(); ?>

	<div id="content">
		<div class="padder">

			<?php do_action( 'bp_before_blog_single_post' ); ?>

			<div class="page" id="blog-single" role="main">

			<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
			

				<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>

					<div class="author-box">
						<?php echo get_avatar( get_the_author_meta( 'user_email' ), '50' ); ?>
						<p><?php printf( _x( 'by %s', 'Post written by...', 'buddypress' ), str_replace( '<a href=', '<a rel="author" href=', bp_core_get_userlink( $post->post_author ) ) ); ?></p>
					</div>

					<div class="post-content">
						<h2 class="posttitle"><?php the_title(); ?></h2>

						<p class="date">
							<?php printf( __( '%1$s <span>in %2$s</span>', 'buddypress' ), get_the_date(), get_the_category_list( ', ' ) ); ?>
							<span class="post-utility alignright"><?php edit_post_link( __( 'Edit this entry', 'buddypress' ) ); ?></span>
						</p>

						<div class="entry">
							<?php the_content( __( 'Read the rest of this entry &rarr;', 'buddypress' ) ); ?>




<!-- pichchapa 12/09/2014 Celeb Cook video player -->



<?php
/* start comment

// check post type
if ( is_singular( 'celebcook' ) ) {
?>


<?php
// then start showing video

global $wpdb;
$pid = get_the_ID(); 
echo "Post ID : ".$pid."<br/>";

// SELECT uploaded video
$results_vup = $wpdb->get_results ( "
    SELECT *
    FROM  $wpdb->postmeta
        WHERE post_id = ".$pid."
		AND meta_key LIKE 'celebcook_upload_video'
" );

foreach ( $results_vup as $res_vup)
{
		// get video id
		$vup = $res_vup->meta_value;
   echo "Upload video ID : ".$res_vup->meta_value."<br/>";
}

// or SELECT video url
$results_vurl = $wpdb->get_results ( "
    SELECT *
    FROM  $wpdb->postmeta
        WHERE post_id = ".$pid."
		AND meta_key LIKE 'celebcook_video_url'
" );

foreach ( $results_vurl as $res_vurl)
{
		// get video id
		$vurl = $res_vurl->meta_value;
   echo "URL video ID : ".$res_vurl->meta_value."<br/>";
}
 
 ?>


<!-- If upload show videojs shortcode / if paste url show youtube player plugin -->
	<div>
		<?php
		
		if($vurl == '' or $vurl == null){
			// videojs plugin
			echo do_shortcode('[videojs mp4="'.wp_get_attachment_url($vup).'" autoplay="false"]');
		}else{
			// youtube player plugin
			echo do_shortcode('[youtube_sc url='.$vurl.']');
		}
		?>
	</div>
	
<?php
}

end comment */ 
?>

<!--end pichchapa 12/09/2014 Celeb Cook video player -->





							<?php wp_link_pages( array( 'before' => '<div class="page-link"><p>' . __( 'Pages: ', 'buddypress' ), 'after' => '</p></div>', 'next_or_number' => 'number' ) ); ?>
						</div>

						<p class="postmetadata"><?php the_tags( '<span class="tags">' . __( 'Tags: ', 'buddypress' ), ', ', '</span>' ); ?>&nbsp;</p>

						<div class="alignleft"><?php previous_post_link( '%link', '<span class="meta-nav">' . _x( '&larr;', 'Previous post link', 'buddypress' ) . '</span> %title' ); ?></div>
						<div class="alignright"><?php next_post_link( '%link', '%title <span class="meta-nav">' . _x( '&rarr;', 'Next post link', 'buddypress' ) . '</span>' ); ?></div>
					</div>

				</div>
				
			
			<!--pichchapa 03/09/2014 wp-email plugin-->
			<?php if(function_exists('email_link')) { email_link(); } ?>
			<!--end pichchapa 03/09/2014 wp-email plugin-->
				
				

			<?php comments_template(); ?>

			<?php endwhile; else: ?>

				<p><?php _e( 'Sorry, no posts matched your criteria.', 'buddypress' ); ?></p>

			<?php endif; ?>

		</div>

		<?php do_action( 'bp_after_blog_single_post' ); ?>

		</div><!-- .padder -->
	</div><!-- #content -->

	<?php get_sidebar(); ?>

<?php get_footer(); ?>