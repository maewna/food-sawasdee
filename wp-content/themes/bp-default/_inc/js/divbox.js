	...
	$.fn.divbox = function(opt){
		...
		function _run(index,init){
			...
			fn.init = function(){
				...
				$(requires).remove();
				$(_cfg.container).prepend('<div id="divbox"></div><div id="divbox_ajax"></div><div id="divbox_frame"><div class="closed" title="'+_cfg.languages.btn_close+'"></div><div id="divbox_data"><div id="divbox_content"></div><div class="marcrot"></div><div class="prev" title="'+_cfg.languages.btn_prev+'"></div><div class="caption"></div><div class="number">10/10</div><div class="next" title="'+_cfg.languages.btn_next+'"></div></div></div>');
			...
			fn.animate = function(t,l,w,h,fncallback,fnclosed, caption){
				...
				$(oFrame).removeClass('white').animate({
					...
					try {
						...
						$("#divbox_frame").draggable({ handle: $(drag_handle) }).css({ cursor: '' });
			...
			fn.viewDefault = function(src,caption){
				...
				fn.animate(top,left,winW,winH,function(o){
					$(o).html('<iframe id="ex3" src="'+src+'" width="715" frameborder="0" scrolling="'+_cfg.scrollbar+'" height="'+winH+'"></iframe>');	
				},false,caption);
			}
