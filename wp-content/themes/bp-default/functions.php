...
function bp_dtheme_comment_form( $default_labels ) {
	?>
	<script src="<?php echo get_template_directory_uri();?>/_inc/js/tinymce/tinymce.min.js"></script>
...
/*edited 'setmenu' to 'set-menu' by pichchapa 12/10/15*/

function create_post_type() {

	register_post_type( 'set-menu',
        array(
            'labels' => array(
                'name' => 'Healthy Set',
                'singular_name' => 'set-menu',
				'add_new' => 'Add New',
			...
			'hierarchical' => false,
			'has_archive' => 'set-menu',
			//'rewrite' => array( 'slug' => 'set-menu', 'hierarchical' => true, 'with_front' => false )
        )
    );
	register_taxonomy( 'setmenu-category', array( 'set-menu' ), 
		array( 
			'labels' => array( 
...
/*edited 'recipes' to 'cooking' by pichchapa 12/10/15*/
    register_post_type( 'cooking',
        array(
            'labels' => array(
                'name' => 'Food @ Home',
                'singular_name' => 'cooking',
                'add_new' => 'Add New',
...
/*edited 'celebcook' to 'celeb-cooking' by pichchapa 12/10/15*/
	register_post_type( 'celeb-cooking',
        array(
            'labels' => array(
                'name' => 'Food Idol',
                'singular_name' => 'celeb-cooking',
                'add_new' => 'Add New',
                'add_new_item' => 'Add New Celeb Recipe',
                'edit' => 'Edit',
                'edit_item' => 'Edit Celeb Recipe',
                'new_item' => 'New Celeb Recipe',
                'view' => 'View',
                'view_item' => 'View Celeb Recipe',
                'search_items' => 'Search',
                'not_found' => 'Not found',
                'not_found_in_trash' => 'Not found in Trash',
                'parent' => 'Parent'
...
/*edited 'tip' to 'kitchen-tips' by pichchapa 12/10/15*/

	register_post_type( 'kitchen-tips',
        array(
            'labels' => array(
                'name' => 'Kitchen Tricks & Tips',
                'singular_name' => 'kitchen-tips',
                'add_new' => 'Add New',
...
/*edited 'bistro' to 'restaurants' by pichchapa 12/10/15*/
...
/*edited 'special_review' to 'reviews' by pichchapa 12/10/15*/
	...
	register_post_type( 'reviews',
        array(
            'labels' => array(
                ...
                'add_new' => 'Add New',
                'add_new_item' => 'Add New Selected Restaurants',
                'edit' => 'Edit',
                'edit_item' => 'Edit Selected Restaurants',
                'new_item' => 'New Selected Restaurants',
                'view' => 'View',
                'view_item' => 'View Selected Restaurants',
                'search_items' => 'Search',
                'not_found' => 'Not found',
                'not_found_in_trash' => 'Not found in Trash',
                'parent' => 'Parent'
...
/*edited 'variety' to 'recommended' by pichchapa 12/10/15*/

	register_post_type( 'recommended',
        array(
            'labels' => array(
                'name' => 'Variety Taste',
                'singular_name' => 'recommended',
                'add_new' => 'Add New',
...
/*edited 'likesara' to 'articles' by pichchapa 12/10/15*/
	...
	register_post_type( 'articles',
        array(
			...
			//'rewrite' => array( 'slug' => articles', 'hierarchical' => true, 'with_front' => false )
        )
    );
	register_taxonomy( 'articles-category', array( 'articles' ), 
		array( 
			'labels' => array( 
			'name' => 'Articles Categories', 
			'menu_name' => 'Articles Categories', 
...
/*
function get_avatar_url($author_id, $args = null ){
$args = get_avatar_data( $author_id, $args );
    return $args['url'];
}*/
...
add_image_size( 'bistroslide', 640, 400, true );
}
...
function render_checkbox_meal( $form_id, $post_id, $form_settings ) {
	$value = array();
	$items = array('breakfast','lunch','dinner','night');
...
//wp_redirect( get_permalink() );
   wp_redirect('http://soba.foodsawasdee.com/add-recipe/');
  exit();
}
...
function lvMember($user_id){
	...
	$resReview = $wpdb->get_results($sqlReview);
	//$numReview = $wpdb->num_rows;
	foreach($resReviews AS $review){
		$reviewId = $review->ID;
		$sqlReviewImg = "SELECT * FROM 'wp_posts' WHERE post_parent='".$reviewId."' ";
		$resReviewImg = $wpdb->get_results($sqlReviewImg);
		$numReviewImg = $wpdb->num_rows;
		if($numReviewImg>1){$qtyReview += 1; }
		else{$review += 1;}
	}
	
	$sqlRecipe = "SELECT * FROM wp_posts WHERE post_type='cooking' AND post_status='publish' AND post_author='".$user_id."'";
	$resRecipe = $wpdb->get_results($sqlRecipe);
	foreach($resRecipe AS $recipe){
		$recipeId = $recipe->ID;
		$sqlRecipeImg = "SELECT * FROM 'wp_posts' WHERE post_parent='".$recipeId."' ";
		$resRecipeImg = $wpdb->get_results($sqlRecipeImg);
		$numRecipeImg = $wpdb->num_rows;
		if($numRecipeImg>1){$qtyRecipe += 1; }
		else{$recipe += 1;}
	}
	//$numRecipe = $wpdb->num_rows;
	$allQtyReview = $qtyReview+$qtyRecipe;
	if($allQtyReview>=10 && $qtyRecipe < 5){
		$ranking = "Starter";
		$rankImg = get_template_directory_uri()."/_inc/images/icons/icon-newbie.png";
	}else if($allQtyReview>=30 && $qtyRecipe>=5){
		$ranking = "Food Spy";
		$rankImg = get_template_directory_uri()."/_inc/images/icons/icon-regular.png";
	}else if($allQtyReview>=60 && $qtyRecipe>=10){
		$ranking = "Superb";
		$rankImg = get_template_directory_uri()."/_inc/images/icons/icon-superb.png";
	}else if($allQtyReview>=100 && $qtyRecipe>=15){
		$ranking = "Elegant";
...
function convertBistroType($bistroTypes){
 	$btypes = array('hotelBistro',
					'ramen',
					'buffet',
					'coffee',
					'barbecue',
	...
	$b = 0;
	$numbistroType = count($bistroTypes);
	$numbistroType = $numbistroType-1;
	foreach($bistroTypes AS $bistroType){

		if (in_array( $bistroType,$btypes)) {
			$key = array_search($bistroType, $btypes);
			$bistroTH =  $btypesTH[$key];
			echo $bistroTH;
			if($b < count($bistroTypes)-1){echo ", ";}
...
function convertFoodType($foodTypes){
 	$types = array(
						 'alacarte',
						 'maincourse',
						 'northfood',
	...
	$b = 0;
	$numbistroType = count($foodTypes);
	$numbistroType = $numbistroType-1;
	foreach($foodTypes AS $foodType){

		if (in_array( $foodType,$types)) {
			$key = array_search($foodType, $types);
			$foodTH =  $typesTH[$key];
			echo $foodTH;
			if($b < count($foodTypes)-1){echo ", ";}
