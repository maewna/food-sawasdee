...
class Agreeable {
	...
	function ag_display_terms_form($type, $errors = '') {
		...
		if(!$this->is_woocommerce_page()) {
			...
			echo '<label style="text-align: left; width:auto; font-size: 14px; margin-top: -80px;margin-left: 55px;"><input type="checkbox" value="1" name="ag_login_accept" id="ag_login_accept" '.$remember.' />&nbsp;<a style="color: #79797c;text-decoration: none;font-weight: normal;" title="'.get_post($this->options['terms_page'])->post_title.'" class="'.$class.'" target="_BLANK" href="'.$term_link.'">'.$this->options['message'].'</a></label>';
