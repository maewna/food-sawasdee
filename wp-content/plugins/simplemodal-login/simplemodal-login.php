	...
	<form name="loginform" id="loginform" action="%s" method="post">
		...
		<p>
			...
			<input type="password" name="pwd" class="user_pass input" value="" size="20" tabindex="20" /></label>
				...
				__('Foodsawasdee', 'simplemodal-login'),
				__('ชื่อผู้ใช้', 'simplemodal-login'),
				__('รหัสผ่าน', 'simplemodal-login')
			);
		...
		<p class="nav">',
				__('จำรหัสผ่าน', 'simplemodal-login'),
				__('เข้าสู่ระบบ', 'simplemodal-login'),
				__('ยกเลิก', 'simplemodal-login')
			);
			...
			if ($this->options['reset']) {
				$output .= sprintf('<a class="simplemodal-forgotpw" href="%s" title="%s">%s</a>',
					...
					__('ลืมรหัสผ่าน?', 'simplemodal-login')
				);
			}
