...
if ( !class_exists( 'WordFilter' ) ) {
	class WordFilter {
		...
		public function valid_modes( $input ) {
			if ( !( in_array( $input, array( WFP_MODE_OFF, WFP_MODE_PASSIVE, WFP_MODE_ACTIVE ) ) ) ) return WFP_MODE_OFF;
			return $input;
		} // end valid_modes

		public function valid_schedules( $input ) {
			if ( !( in_array( $input, array( WFP_SCHEDULE_FIVE, WFP_SCHEDULE_FIFTEEN, WFP_SCHEDULE_THIRTY, WFP_SCHEDULE_SIXTY ) ) ) ) return WFP_SCHEDULE_SIXTY;
			return $input;
		} // end valid_schedules
		...
		private function do_replacement( $original='', $replacement='', $case_insensitive='', $partial_match='', $use_regex='', $string='' ) {
			if( $partial_match != 'yes' ) 	$b ='\b';
			if( $case_insensitive == 'yes' ) 	$i ='i';
		...
		function schedule_batch( $event ){		
			...
			if ( ! wp_next_scheduled( $event ) ) {
				...
				if ( WFP_DEBUG ) error_log("ALREADY scheduled:$scheduledInterval, $event");
			}
			if ( $next_event = wp_next_scheduled( $event ) ) {
				if ( WFP_DEBUG ) error_log("Now scheduled:  " . date( 'h:i:s', $scheduled_for) . ", " . date( 'h:i:s', $next_event) . ", $event");
			} else {
				if ( WFP_DEBUG ) error_log("Still NOT scheduled: $scheduledInterval, $event");
			}
		} // end schedule_batch

		function cancel_batch( $event ){
			global $wpdb;
			if ( WFP_DEBUG ) error_log("Now canceled:  $event");
			if ( wp_next_scheduled( $event ) ) {
		...
		function process_wfpPostEvent() {
			if ( get_transient( 'batch_running' ) ) {
				if ( WFP_DEBUG ) error_log( "Batch already running. Exiting" );
				exit;
			}
			set_transient( 'batch_running', true );
			global $wpdb;
		
			if ( WFP_DEBUG ) error_log( "!!!!!!!!!!!!!!IN process_wfpPostEvent" );
			...
			if ( WFP_DEBUG ) error_log( "sql: $sql" );

			$record_count = mysql_num_rows( $result );
			if ( !$result || ( $record_count === 0 ) ) {
				// Exit out with no errors.  End batch.
				if ( WFP_DEBUG ) error_log( "Exiting. No Results" );
			...
			while ( $row = mysql_fetch_assoc( $result ) ) {
				...
				if ( $cancelled > 0 ) {
					...
					if ( $nts < time() ) {
					if ( WFP_DEBUG ) error_log( "Exceeded time limit. Exiting:  " . date( 'h:i:s', time() ) . ", " . date( 'h:i:s', $nts ) );
						delete_transient( 'batch_running' );
						exit;
					}
				}
				$cur_id = $row[ 'ID' ];
				if ( WFP_DEBUG ) error_log( "Processing $cur_id: " );
				...
				*/
				...
				if ( ( $new_post_content <> $row[ 'post_content' ] ) || ( $new_post_excerpt <> $row[ 'post_excerpt' ] ) || ( $new_post_title <> $row[ 'post_title' ] ) ) {
					if ( WFP_DEBUG ) error_log( "Update Required for $cur_id: " );
			...
			if ( $record_count < $wfp_batch_size ) {
				...
				if ( WFP_DEBUG ) error_log( "Ending Scheduled Batch. We're done." );
				wp_clear_scheduled_hook( WFP_POST_EVENT );
				delete_option( WFP_OPTION_ITERATION );
			}
			if ( WFP_DEBUG ) error_log( "Batch Done. Exiting" );
			delete_transient( 'batch_running' );
			...
			while ( $row = mysql_fetch_assoc( $result ) ) {
				...
				*/
				...
				if ( ( $new_comment_content <> $row[ 'comment_content' ] ) ) {
					if ( WFP_DEBUG ) error_log( "Update Required for comment: $cur_id: " );
