        ...
        if ( $post_id ) {
            ...
            if ( $is_update ) {
                if ( $form_settings['edit_redirect_to'] == 'page' ) {
					...
					if($form_settings['post_type']=='review'){
						global $wpdb;
						$sql = "SELECT * FROM  wp_postmeta WHERE post_id='".$postarr[ID]."' AND  meta_key='review_bistroid'";
						$results = $wpdb->get_results($sql);
						$redirect_to =  get_permalink($results[0]->meta_value)."#review_".$postarr['ID'];
						/*if (isset($_SESSION['pid'])) {
							$redirect_to = get_permalink($_SESSION['pid']);
							
							unset($_SESSION['pid']); 
							// do stuff
						} */
					}
					else if($form_settings['post_type']=='recipes'){
						$redirect_to = get_permalink($postarr['ID']);
					}
					...
					//$redirect_to = "http://www.sanook.com";
                } else {
                    $redirect_to = get_permalink( $post_id );
					if($form_settings['post_type']=='review'||$form_settings['post_type']=='bistroimage'){
						global $wpdb;
						$sql = "SELECT * FROM  wp_postmeta WHERE post_id='".$postarr[ID]."' AND  meta_key='review_bistroid'";
						$results = $wpdb->get_results($sql);
						$redirect_to =  get_permalink($results[0]->meta_value)."#review_".$postarr['ID'];
					...
					//$redirect_to = "http://www.gmail.com";
                }
