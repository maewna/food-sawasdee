...
class WPUF_Render_Form {
    ...
    function render_form( $form_id, $post_id = NULL, $preview = false ) {
        ...
        if ( ! is_user_logged_in() && $form_settings['guest_post'] != 'true' ) {
			...
			echo "var chk = '$checkLogin';"; 
  			echo '</script>';
			?>
			<script type="text/javascript">
			  $(document).ready(function(){
				  if(chk == 'f'){
					 window.location.href = "<?php echo get_site_url(); ?>/wp-login.php";
					}
				});
			</script>
    ...
    function submit_button( $form_id, $form_settings, $post_id ) {
        ...
        <li class="wpuf-submit">
            ...
            <?php } ?>
             ...
             <input type="button" value="Cancel" onclick="confirmCancel('<?php echo $form_id;?>')" />
   
        </li>
        <script>
		function confirmCancel(v) {
			var txt;
			if(v == '258'){
					txt = "การเพิ่มสูตรอาหาร";
				} else if(v == '200'){
					txt = "การเพิ่มร้านอาหาร";	
				} else if(v == '73'){
					txt = "การเพิ่มรีวิว";	
				} else {
					txt = ""
			...
			if (confirm("ต้องการยกเลิก"+txt+"ใช่หรือไม่") == true) {
