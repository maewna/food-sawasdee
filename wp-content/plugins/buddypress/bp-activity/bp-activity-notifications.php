...
function bp_activity_format_notifications( $action, $item_id, $secondary_item_id, $total_items, $format = 'string' ) {
	...
	switch ( $action ) {
		...
		case 'new_at_recipes':
			...
			$user_display = bp_core_get_user_displayname( $activity_id );
			
			$post = get_post($post_id); 
			$postTitle = $post->post_title;
				
			$at_mention_link  = $post->guid;
			$at_mention_title = sprintf( __( '%s', 'buddypress' ), $postTitle );
				
			    if ( (int) $total_items > 1 ) {
					$text = sprintf( __( 'มีการเพิ่มสูตรอาหาร %1$d สูตร', 'buddypress' ), (int) $total_items );
					$filter = 'bp_activity_multiple_new_at_recipe_notification';
				} else {	
					$text = sprintf( __( '%s เพิ่มสูตรอาหาร " %s "', 'buddypress' ), $user_display, $postTitle );
					$filter = 'bp_activity_single_new_at_recipe_notification';
				}  
			

		break; 
		case 'edit_at_recipes':
			$activity_id    = $item_id;
			$post_id   		= $secondary_item_id;
			$user_display = bp_core_get_user_displayname( $activity_id );
			...
			$at_mention_title = sprintf( __( '%s', 'buddypress' ), $postTitle );
				...
				if ( (int) $total_items > 1 ) {
					$text = sprintf( __( 'มีการแก้ไขสูตรอาหาร %1$d สูตร', 'buddypress' ), (int) $total_items );
					$filter = 'bp_activity_multiple_edit_at_recipe_notification';
				} else {
					$text = sprintf( __( '%s แก้ไขสูตรอาหาร " %s "', 'buddypress' ), $user_display, $postTitle );
					$filter = 'bp_activity_single_edit_at_recipe_notification';
				}
		...
		case 'new_at_bistro':
			...
			$at_mention_title = sprintf( __( '%s', 'buddypress' ), $postTitle );
			    ...
			    if ( (int) $total_items > 1 ) {
					$text = sprintf( __( 'มีการเพิ่มร้านอาหาร %1$d ร้าน', 'buddypress' ), (int) $total_items );
					$filter = 'bp_activity_multiple_new_at_bistro_notification';
				} else {	
					$text = sprintf( __( '%s เพิ่มร้านอาหาร " %s "', 'buddypress' ), $user_display, $postTitle );
					$filter = 'bp_activity_single_new_at_bistro_notification';
				}  
		...
		case 'edit_at_bistro':
			...
			$at_mention_title = sprintf( __( '%s', 'buddypress' ), $postTitle );
				...
				if ( (int) $total_items > 1 ) {
					$text = sprintf( __( 'มีการแก้ไขร้านอาหาร %1$d ร้าน', 'buddypress' ), (int) $total_items );
					$filter = 'bp_activity_multiple_edit_at_bistro_notification';
				} else {
					$text = sprintf( __( '%s แก้ไขร้านอาหาร " %s "', 'buddypress' ), $user_display, $postTitle );
					$filter = 'bp_activity_single_edit_at_bistro_notification';
				}
		...
		case 'new_at_review':
			...
			$at_mention_title = sprintf( __( '%s', 'buddypress' ), $postTitle );
			    ...
			    if ( (int) $total_items > 1 ) {
					$text = sprintf( __( 'มีการเพิ่มรีวิว %1$d รีวิว', 'buddypress' ), (int) $total_items );
					$filter = 'bp_activity_multiple_new_at_review_notification';
				} else {	
					$text = sprintf( __( '%s เพิ่มรีวิว " %s "', 'buddypress' ), $user_display, $postTitle );
					$filter = 'bp_activity_single_new_at_review_notification';
				}  
		...
		case 'edit_at_review':
			...
			$at_mention_title = sprintf( __( '%s', 'buddypress' ), $postTitle );
				...
				if ( (int) $total_items > 1 ) {
					$text = sprintf( __( 'มีการแก้ไขรีวิว %1$d รีวิว', 'buddypress' ), (int) $total_items );
					$filter = 'bp_activity_multiple_edit_at_review_notification';
				} else {
					$text = sprintf( __( '%s แก้ไขรีวิว " %s "', 'buddypress' ), $user_display, $postTitle );
					$filter = 'bp_activity_single_edit_at_review_notification';
				}
		...
		case 'new_at_post':
			$activity_id    = $item_id;
			$post_id   		= $secondary_item_id;
			$user_fullname = bp_core_get_user_displayname( $activity_id );
			...
			$postTypeTH = '';
				...
				if($postType == 'recipes'){
					$postTypeTH = 'สูตรอาหาร';
					$filter = 'bp_activity_at_recipe_post_notification';
				}else if($postType == 'bistro'){
					$postTypeTH = 'ร้านอาหาร';
					$filter = 'bp_activity_at_bistro_post_notification';
				}else if( $postType == 'review'){
					$postTypeTH = 'รีวิว';	
					$filter = 'bp_activity_at_review_post_notification';
				}
			    ...
			    if ( (int) $total_items > 1 ) {
					$text = sprintf( __( 'มีการสร้าง %1$d สูตร', 'buddypress' ), (int) $total_items );
					$filter = 'bp_activity_multiple_at_post_notification';
				} else {	
					$text = sprintf( __( '%s สร้าง%s " %s "', 'buddypress' ), $user_fullname, $postTypeTH, $postTitle );
					$filter = 'bp_activity_single_at_post_notification';
				}  
		...
		case 'edit_at_post':
			...
			$postTypeTH = '';
				...
				if ( (int) $total_items > 1 ) {
					$text = sprintf( __( 'มีการแก้ไข %1$d สูตร', 'buddypress' ), (int) $total_items );
					$filter = 'bp_activity_multiple_at_post_notification';
				} else {
					$text = sprintf( __( '%s แก้ไข%s " %s "', 'buddypress' ), $user_fullname, $postTypeTH, $postTitle );
					$filter = 'bp_activity_single_at_post_notification';
				}
