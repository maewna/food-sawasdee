	...
	public function __construct( $args = array() ) {
		...
		if ( (int) $this->total_notification_count && (int) $this->pag_num ) {
			$this->pag_links = paginate_links( array(
				...
				'prev_text' => _x( 'ก่อนหน้า', 'Notifications pagination previous text', 'buddypress' ),
				'next_text' => _x( 'ถัดไป', 'Notifications pagination next text',     'buddypress' ),
				'mid_size'  => 1,
			) );
...
function bp_the_notification_delete_link() {
	...
	function bp_get_the_notification_delete_link() {
		...
		<a href="<?php echo esc_url( $url ); ?>" class="delete secondary confirm"><?php _e( '<img src="/wp-content/themes/bp-default/_inc/images/profile/close-non-unread.png" style="  position: absolute;">', 'buddypress' ); ?></a>
...
function bp_notification_has_more_items() {
	echo bp_get_the_notification_has_more_items();
}
	function bp_get_the_notification_has_more_items() {
	// Setup local variables
		//$noti_more  = '';
		$bp_more    = buddypress();
		$notification_more = $bp_more->notifications->query_loop->notification;
	
	
	$remaining_pages = 0;

	if ( ! empty( $notification_more->pag_page ) ) {
		$remaining_pages = floor( ( $notification_more->total_notification_count - 1 ) / ( $notification_more->pag_num * $notification_more->pag_page ) );
	}

	$noti_more  = (int) $remaining_pages ? true : false;

	return apply_filters( 'bp_get_the_notification_has_more_items', $noti_more );
}	
